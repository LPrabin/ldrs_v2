...
def extract_toc_content(content, model=None):
    ...
    Directly return the full table of contents content.Table of contents may span multiple pages. Do not output anything else."""
...
def toc_transformer(toc_content, model=None):
    ...
    print('last_complete', last_complete)
...
def generate_toc_continue(toc_content, part, model="qwen3-vl"):
    print('start generate_toc_continue')
    
    # Context Optimization: Only show the last 20 items to maintain continuity
    # This prevents the prompt from exploding as the TOC grows
    toc_history = toc_content
    if isinstance(toc_content, list) and len(toc_content) > 20:
        toc_history = toc_content[-20:]
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    # Simply concatenate all page content without token-based splitting
    # Let the LLM backend handle token limits naturally
    page_contents = []
    for page_index in range(start_index, start_index+len(page_list)):
        page_text = f"<physical_index_{page_index}>\n{page_list[page_index-start_index][0]}\n<physical_index_{page_index}>\n\n"
        page_contents.append(page_text)
    
    # Join all content - backend will handle token limits
    full_text = "".join(page_contents)
    logger.info(f'Processing full text length: {len(full_text)} chars')
    
    toc_with_page_number = generate_toc_init(full_text, model)
...
def process_toc_no_page_numbers(toc_content, toc_page_list, page_list,  start_index=1, model=None, logger=None):
    # Simply concatenate page content without token-based splitting
    # Let the LLM backend handle token limits naturally
    ...
    toc_with_page_number = add_page_number_to_toc(full_text, toc_with_page_number, model)
...
################### fix incorrect toc #########################################################
def single_toc_item_index_fixer(section_title, content, model="qwen3-vl"):
    tob_extractor_prompt = """
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None):
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node or token_num >= opt.max_token_num_each_node:
        ...
        # Process the node range in smaller chunks to avoid token limits
        all_toc_items = []
        current_start = node['start_index']
        
        while current_start <= node['end_index']:
            # Calculate chunk bounds
            chunk_end = min(current_start + opt.max_page_num_each_node - 1, node['end_index'])
            chunk_page_list = page_list[current_start-1:chunk_end]
            
            # Process this chunk
            chunk_toc = await meta_processor(chunk_page_list, mode='process_no_toc', start_index=current_start, opt=opt, logger=logger)
            chunk_toc = await check_title_appearance_in_start_concurrent(chunk_toc, page_list, model=opt.model, logger=logger)
            
            # Filter valid items
            valid_items = [item for item in chunk_toc if item.get('physical_index') is not None]
            all_toc_items.extend(valid_items)
            
            # Move to next chunk - skip pages we've already processed
            current_start = chunk_end + 1
        
        # Post-process all collected items
        if all_toc_items and node['title'].strip() == all_toc_items[0]['title'].strip():
            node['nodes'] = post_processing(all_toc_items[1:], node['end_index'])
            node['end_index'] = all_toc_items[1]['start_index'] if len(all_toc_items) > 1 else node['end_index']
        else:
            node['nodes'] = post_processing(all_toc_items, node['end_index'])
            node['end_index'] = all_toc_items[0]['start_index'] if all_toc_items else node['end_index']
